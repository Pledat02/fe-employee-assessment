...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const allMenuItems: MenuItem[] = [
    ...
    { id: 'evaluation', label: 'Đánh giá nhân viên', path: '/evaluation', icon: 'fas fa-star', permission: 'evaluation' },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`bg-gradient-to-b from-white to-gray-50 shadow-xl w-64 h-screen flex flex-col fixed top-0 left-0 z-50 transition-transform duration-300 border-r border-gray-200 ${
        isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
      {/* Header */}
      <div className="flex items-center justify-center p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-blue-700">
        <div className="text-center">
          <span className="text-xl font-bold text-white">HR Evaluation</span>
          <div className="text-xs text-blue-100 mt-1">Hệ thống đánh giá</div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 p-4 bg-gray-50/50">
        {/* User Profile */}
        <div className="flex items-center p-3 rounded-xl bg-white shadow-sm mb-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg">
        ...
        <button
          ...
          className="flex items-center w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors duration-200 group rounded-xl border border-gray-200 bg-white shadow-sm"
        >
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 group-hover:bg-red-100 mr-3 transition-colors">
